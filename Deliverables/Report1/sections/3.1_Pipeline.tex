...
\noindent Once the dataset is preprocessed, we setup the Optuna study, connecting all the computational resources (e.g. local, Kaggle and Colab GPUs) to the same experiment in the Database.
...
Optuna chooses also to include or not global features, whether to weight the loss with the class weights and to perform or not windowing.

For each sampled configuration, we apply K-fold cross-validation with $K=5$. The pipeline, using the dataloader class, splits train and validation data, including the test data if the sampled macro-architecture is
an AE and optionally loads the global features.
The architecture is built using the sampled configuration and trained. We used \href{https://lightning.ai/}{Pytorch Lightning} as a way to simplify model training, checkpointing and logging.
...
Optionally, we can retrain the best model or create an ensemble model where each submodel is the best checkpoint for every fold.
