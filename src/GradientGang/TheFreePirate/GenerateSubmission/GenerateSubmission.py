...
def generateSubmission(best_params, csv_path):
    ...
    testSet = data.getTestLoader(
        windowAugmenter=WindowingAugmenter(
            windowSize=dataAug["windowSize"], stride=dataAug["windowStride"]
        )
    )
    ...
    with torch.no_grad():
        for i in range(numFolds):
            ...
            fold_model_path = (
                f"{best_model_path}\\fold_{i}"  # PRENDERE L'ULTIMO FILE DELLA DIRECTORY
            )
    ...
    submission_df = pd.DataFrame(
        {"sample_index": sample_indices, "label": textual_predictions}
    )
